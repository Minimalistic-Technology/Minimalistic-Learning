...
const totalAuthors = Array.from(
  new Set(
    normalizedBlogs.map((blog) => blog.author ?? "Minimalistic Learning"),
  ),
).length;
const averageRating =
  normalizedBlogs.length > 0
    ? (
        normalizedBlogs.reduce((sum, blog) => sum + (blog.rating ?? 5), 0) /
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50/50 to-pink-50/50 dark:from-slate-950 dark:via-purple-950/20 dark:to-indigo-950/20 text-slate-800 dark:text-slate-100">
      ...
      <div className="container h-screen mx-auto px-4 py-16 flex items-center justify-center text-center font-semibold text-2xl md:text-3xl lg:text-4xl">
        <div className="flex flex-col gap-2">
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6">
            Coming Soon
          </h1>
          <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-300 max-w-2xl leading-relaxed">
            A new home for Minimalistic Learning! We're working hard to bring
            you an even better experience. Stay tuned for updates and get ready
            to explore a world of knowledge with us. Thank you for being part of
            our journey!
          </p>
        </div>
      </div>
