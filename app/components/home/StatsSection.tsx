...
function StatItem({ number, label, icon, index }: StatItemProps) {
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-sky-500/5 to-cyan-500/5 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="relative z-10 flex flex-col items-center text-center space-y-4">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-100 to-sky-100 dark:from-blue-900/30 dark:to-sky-900/30 group-hover:scale-110 transition-transform duration-300">
          {icon}
        </div>
        <div>
          <motion.span
            ...
            className="block text-4xl md:text-5xl font-extrabold bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 dark:from-slate-100 dark:via-blue-100 dark:to-slate-100 bg-clip-text text-transparent"
          >
            {number}
          </motion.span>
...
export default function StatsSection({ 
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: <Star className="w-8 h-8 text-blue-500 fill-blue-500" />,
    },
  ];
